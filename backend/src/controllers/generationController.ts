...
import { textToImage, imageToImage, imageToVideo } from '../services/falService';
...
export const generateTextToImage = async (req: Request, res: Response) => {
  try {
    ...
    if (isNsfw && user.plan === 'FREE') {
      return res.status(403).json({ error: 'Plan FREE no permite contenido NSFW' });
    }
...
export const generateImageToVideo = async (req: Request, res: Response) => {
  try {
    ...
    if (user.plan === 'FREE') {
      return res.status(403).json({ error: 'Plan FREE no permite generación de video' });
    }
