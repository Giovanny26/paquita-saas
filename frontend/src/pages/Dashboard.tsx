  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <h2 className="text-3xl font-bold mb-2">Hola, {user?.name || user?.email} 👋</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-zinc-400 text-sm">Edita y transforma imágenes existentes con IA.</p>
            <span className="text-purple-400 text-sm mt-4 block">1 crédito / imagen</span>
          </div>
        ...
        {/* Avatar section */}
        <div
          onClick={() => user?.plan !== 'FREE' && navigate('/avatar')}
          className={`mt-10 bg-gradient-to-r from-purple-900/20 to-pink-900/20 border border-purple-500/20 rounded-2xl p-6 ${
            user?.plan !== 'FREE' ? 'cursor-pointer hover:border-purple-500/40 transition' : ''
          }`}
        >
